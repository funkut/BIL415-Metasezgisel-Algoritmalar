...
%   BestCostHistory : Convergence curve
    ...
    MaxIt        = params.MaxIt;
    nFood        = params.nFoodSources;  % gıda kaynağı sayısı (çözüm sayısı)
    Limit        = params.Limit;         % geliştirme limiti
    ...
    % Baslangic kaynaklari (rastgele turlar)
    for i = 1:nFood
    ...
    for it = 1:MaxIt
        ...
        %% 1. Employed bees fazi (isci arilar)
        for i = 1:nFood
            % Rastgele baska bir kaynak sec (k != i)
            k = randi(nFood);
            while k == i
                k = randi(nFood);
            end
            ...
            % Komsu cozum üret (swap mutasyon)
        ...
        %% 3. Onlooker bees fazi (gozlemci arilar)
        ...
        for i = 1:nFood
            if food(i).Trials >= Limit
                % Cozumu bastan rastgele olustur
                food(i).Position = randperm(nCities);
                food(i).Cost     = TSPCost(food(i).Position, problem);
                food(i).Trials   = 0;
            end
...
function p_new = SwapMutation(p)
    % TSP turu icin basit swap mutasyonu
    n = numel(p);
    i = randi(n);
    j = randi(n);
    while j == i
...
function idx = RouletteWheelSelection(p)
    % Rulet tekerlegi secimi (olasilik vektorune gore index sec)
    r = rand;
    c = cumsum(p);
